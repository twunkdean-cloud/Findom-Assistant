...
serve(async (req) => {
  ...
  try {
    ...
    if (!image || !mimeType || !prompt) {
      return new Response(JSON.stringify({ error: 'Image, mimeType, and prompt are required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!response || !response.ok) {
      return new Response(JSON.stringify({ error: `All vision models failed. Last error: ${lastError?.error?.message || lastError?.error || 'Unknown error'}` }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!generatedText) {
      return new Response(JSON.stringify({ error: 'No response generated from Gemini Vision' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
