...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to generate content', debug: errorData }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const data = await response.json();
    const generatedText = data.candidates?.[0]?.content?.parts?.[0]?.text || '';

    return new Response(JSON.stringify({ text: generatedText }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error: any) {
